...
@app.route('/ownerlogin/ownerhome', methods=['GET', 'POST'])
def owner_home():
...
# @app.route('/customerlogin/taxi', methods=['GET', 'POST', 'PUT', 'DELETE'])
# def add_taxi_page():
#     # items = taxi.query.all()
#     form = selectTaxiForm()
#     if form.validate_on_submit():
#         cab = taxi(driver_name=form.driver_name.data, registration_no=form.registration_no.data,
#                    taxi_type=form.taxi_type.data, From=form.From.data, To=form.To.data,
#                    flag=0, owner_id=form.owner_id.data)   #   ??????
#         db.session.add(cab)
#         db.session.commit()
